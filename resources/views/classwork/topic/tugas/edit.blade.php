...
@section('content')
    <div class="flex container-index w-full py-0">
        <div class="flex-1">
            <div class="py-5 pr-5">
                <form id="taskForm"
                    action="{{ route('classwork.prinsip.tugas.update', ['topic' => $topic, 'id' => $modul->id, 'task_id' => $task->id]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-10 flex flex-col gap-3">
                        <label class="text-2xl">Title</label>
                        <input type="text" name="title" class="border-b max-w-xl outline-none focus:outline-none" value="{{ $task->title }}">
                    </div>
                    <div class="mb-10 flex flex-col gap-3">
                        <label class="text-2xl">Deskripsi</label>
                        <textarea id="deskripsi" name="description" class="border-b max-w-xl" rows="10">{{ $task->description }}</textarea>
                    </div>
                    <!-- Hidden container for criteria inputs -->
                    <div id="hiddenCriteriaContainer">
                        @foreach ($task->kriteria_penilaian as $index => $kriteria)
                            <input type="hidden" name="kriteria_penilaian[]" value="{{ $kriteria }}"
                                data-id="{{ $index }}">
                        @endforeach
                    </div>
                    <input type="hidden" name="due_date" id="hiddenDueDate" value="{{ $task->due_date }}">
                </form>
            </div>
        </div>
        <div class="min-w-72 max-w-md sticky top-32 h-5/6">
            <div class="border-l pl-5 flex h-full flex-col gap-10 py-5">
                <div class="mb-6 w-full">
                    <h1 class="text-2xl mb-4">Due Date</h1>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const hiddenCriteriaContainer = $('#hiddenCriteriaContainer');
            const hiddenDueDate = $('#hiddenDueDate');

            // Ensure due date is updated in the hidden field
            hiddenDueDate.val($('#dueDate').val());

            // Update hidden due date input when date changes
