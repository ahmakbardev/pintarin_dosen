...
@section('content')
    <div class="flex container-index w-full py-0">
        <div class="flex-1">
            <div class="py-5">
                <form
                    action="{{ route('classwork.topic.materi.update', ['topic' => $topic, 'id' => $modul->id, 'materi_id' => $materi->id]) }}"
                    method="POST" enctype="multipart/form-data" id="materiForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-10 flex flex-col gap-3">
                        <label class="text-2xl">Title</label>
                        <input type="text" name="title" class="border-b max-w-xl px-3 py-2 outline-none"
                            value="{{ $materi->title }}" required>
                    </div>
                    <div class="mb-10 flex flex-col gap-3 text-sm">
                        <label class="text-2xl">Materi</label>
                        <textarea id="hidden-materi" name="content" style="display:none" required>{{ $materi->content }}</textarea>
                        <div id="materi-editor" class="border-b max-w-xl" rows="10">{!! $materi->content !!}</div>
                    </div>
                    ...
                    <input type="hidden" id="urlInputHidden" name="url" value="{{ $materi->url }}">
                    <input type="hidden" id="fileInputHidden" name="file" value="{{ $materi->file }}">
                    ...
                    <div class="flex justify-end my-5">
                        <button type="submit"
                            class="py-3 px-5 text-lg bg-primary text-white rounded-lg hover:ring transition-all ease-in-out">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        CKEDITOR.ClassicEditor.create(document.querySelector('#materi-editor'), {
            ...
            // Event listener to sync CKEditor content with hidden textarea
            $('#materiForm').on('submit', function() {
                $('#hidden-materi').val(editor.getData());
            });
        }).catch(error => {
            console.error(error);
        });
        ...
        $(document).ready(function() {
            ...
            $('#urlForm').on('submit', function(event) {
                ...
                setTimeout(() => {
                    $('#urlPreviewContainer .relative').removeClass(
    ...
    `;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        fileModal.removeClass('opacity-100');
                        fileModal.find('> div').addClass('translate-y-10');
                        fileModal.find('> div').removeClass(
                            'translate-y-0');
                        $('#fileInputHidden').val(
    ...
    `;
            ...
            // Display initial previews if values exist
            const initialUrl = '{{ $materi->url }}';
            if (initialUrl) {
                updateUrlPreview({
                    url: initialUrl,
                    title: '{{ $materi->url_title }}',
                    description: '{{ $materi->url_description }}',
                    image: '{{ $materi->url_image }}'
                });
            }

            const initialFile = '{{ $materi->file }}';
            if (initialFile) {
                updateFilePreview({
                    url: initialFile,
                    title: '{{ $materi->file_title }}'
                });
            }
