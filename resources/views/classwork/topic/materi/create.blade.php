...
@section('content')
        ...
        <div class="flex-1">
            <div class="py-5">
                <form action="{{ route('classwork.topic.materi.store', ['topic' => $topic, 'id' => $modul->id]) }}"
                    method="POST" enctype="multipart/form-data" class="my-10" id="materiForm">
                    @csrf
                    ...
                    <div class="mb-10 flex flex-col gap-3 text-sm">
                        ...
                        <textarea id="hidden-materi" name="content" style="display:none" required></textarea>
                        <div id="materi-editor" class="border-b max-w-xl" rows="10"></div>
                    </div>
                    <div class="flex gap-5">
                        <button type="button" id="urlButton" class="relative group">
                        ...
                        <button type="button" id="fileButton" class="relative group">
                            <svg class="w-6 h-6" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <form id="urlForm">
                ...
                <div class="flex justify-end gap-3 mt-8">
                    <button type="button" id="closeUrlModal"
                        class="px-4 py-2 bg-white hover:ring rounded-lg transition-all ease-in-out">Batal
                    </button>
                    <button type="submit"
                        class="px-4 py-2 bg-primary text-white hover:bg-blue-600 hover:ring transition-all ease-in-out rounded-lg">Simpan
                    </button>
                </div>
            </form>
    ...
    <script>
        CKEDITOR.ClassicEditor.create(document.querySelector('#materi-editor'), {
            toolbar: {
                items: [
        ...
        document.querySelector('#materiForm').addEventListener('submit', function(event) {
            if (window.editor) {
                ...
                const hiddenTextarea = document.querySelector('#hidden-materi');
                hiddenTextarea.value = content;

                // Get URL from URL preview
                const hiddenUrl = document.querySelector('#hidden-url');
                const urlPreview = document.querySelector('#urlPreviewContainer .text-primary');
                if (urlPreview) {
                    hiddenUrl.value = urlPreview.href;
                }

                // Get File URL from file preview
                const hiddenFile = document.querySelector('#hidden-file');
                const filePreview = document.querySelector('#filePreviewContainer .text-primary');
                if (filePreview) {
                    hiddenFile.value = filePreview.href;
                }
            }
        });
        ...
        $(document).ready(function() {
            ...
            // Adjust URL form submission
            $('#urlForm').on('submit', function(event) {
                event.preventDefault();
                const urlInput = $('#urlInput').val();
                $('#hidden-url').val(urlInput); // Set the hidden input field with the URL
                isLoading = true;
            ...
            $(document).on('click', '#deleteUrl', function(event) {
                ...
                $('#urlInput').val('');
                $('#hidden-url').val(''); // Clear the hidden input field as well
                $('#urlPreviewContainer').html('');
            });
            ...
            $(document).on('click', '#deleteFile', function(event) {
                ...
                event.stopPropagation();
                $('#fileInput').val('');
                $('#hidden-file').val(''); // Clear the hidden input field as well
            ...
            $('#fileForm').on('submit', function(event) {
                ...
                const initialPreviewHtml = ` <
                        div class =
                        "relative flex gap-5 ring-1 ring-black rounded-2xl my-4 p-4 opacity-0 translate-y-10 transition-all duration-1000 ease-in-out overflow-hidden" >
                        <
                        img src = "{{ asset('assets/images/thumbnail/default.png') }}"
                        alt = ""
                        class = "h-full aspect-video max-w-48 object-cover" >
                        <
                        div class = "flex justify-between w-full items-center" >
                        <
                        div class = "flex flex-col gap-3" >
